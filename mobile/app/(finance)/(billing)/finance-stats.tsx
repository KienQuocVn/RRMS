...
const STAT_CONFIGS: Record<FinanceStatType, { title: string; alertText: string; alertIconColor: string; alertBg: string; alertBorder: string }> = {
  debt: {
    title: 'Thống kê hóa đơn đang nợ và chưa thu',
    alertText: 'Thống kê danh sách phòng hiện tại Chưa thu tiền - Khách nợ tiền thuê',
...
export default function FinanceStatsScreen() {
  ...
  const AlertBanner = () => (
    <View style={[styles.alertBanner, { backgroundColor: config.alertBg, borderColor: config.alertBorder }]}>
      <Ionicons name="warning" size={24} color={config.alertIconColor} style={{ marginRight: 8 }} />
      <Text style={[styles.alertText, { color: config.alertIconColor }]}>
        {config.alertText.split(currentType === 'debt' ? 'Chưa thu tiền - Khách nợ tiền thuê' : '...').map((part, index, array) => {
          if (index === array.length - 1) return part;
          return (
            <React.Fragment key={index}>
              {part}
              <Text style={{ fontWeight: 'bold' }}>{currentType === 'debt' ? 'Chưa thu tiền - Khách nợ tiền thuê' : ''}</Text>
            </React.Fragment>
          );
        })}
      </Text>
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={styles.infoCol}>
          <Text style={styles.roomTitle}>Phòng 1 <Text style={styles.dateSubtext}>(18/04/2026)</Text></Text>
          <Text style={styles.descText}>Thu tiền hàng tháng</Text>
          
          <View style={styles.appStatusRow}>
            <Ionicons name="information-circle-outline" size={14} color="#F57C00" style={{ marginRight: 4 }} />
      ...
      <View style={styles.actionGrid}>
        <TouchableOpacity style={[styles.actionGridBtn, styles.actionCall]}>
          <Ionicons name="call" size={16} color="#2196F3" style={{ marginRight: 6 }} />
          <Text style={[styles.actionGridText, { color: '#2196F3' }]}>Gọi</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionGridBtn}>
          <Ionicons name="cash-outline" size={16} color={Colors.textPrimary} style={{ marginRight: 6 }} />
        ...
        <TouchableOpacity style={[styles.actionGridBtn, { borderRightWidth: 0 }]}>
          <Ionicons name="print-outline" size={16} color={Colors.textPrimary} style={{ marginRight: 6 }} />
      ...
      <View style={styles.moneySummary}>
        ...
        <View style={[styles.moneyCol, { backgroundColor: '#E8F5E9', marginHorizontal: 8 }]}>
        ...
        <View style={[styles.moneyCol, { backgroundColor: '#FFEBEE' }]}>
          ...
          <Text style={[styles.moneyValue, { color: '#F44336' }]}>3.000.000 đ</Text>
        </View>
      </View>
    </View>
  );

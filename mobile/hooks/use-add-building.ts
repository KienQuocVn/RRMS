...
export function useAddBuilding() {
  ...
  const rentType = useAddBuildingFlow((state) => state.basicInfo.rentType);
  const setRentType = useAddBuildingFlow((state) => state.setRentType);
  const isAutoInit = useAddBuildingFlow((state) => state.basicInfo.isAutoInit);
  const setIsAutoInit = useAddBuildingFlow((state) => state.setIsAutoInit);

  const handleNext = () => {
    router.push('/menu-management-home/add-building/add-building-services');
  };

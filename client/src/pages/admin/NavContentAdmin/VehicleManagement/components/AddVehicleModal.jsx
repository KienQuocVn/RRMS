...
const AddVehicleModal = ({ open, onClose, rooms, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchTenants = async () => {
      ...
      try {
        const roomTenants = await getTenantsByRoomId(selectedRoomId)
        ...
        if (roomTenants.length > 0) {
          ...
          toast.info('Phòng này chưa có khách thuê')
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(error)
      const message = error?.response?.data?.message || 'Có lỗi xảy ra khi thêm phương tiện'
      toast.error(message)
    } finally {
      setLoading(false)
    }
            ...
            <Box sx={{ mb: 2 }}>
              <Box sx={{ borderLeft: `3px solid ${Colors.info}`, pl: 1, mb: 1 }}>
                <Typography variant="subtitle2" fontWeight="bold">
                  Chọn phòng sử dụng xe
                </Typography>
              </Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2, fontStyle: 'italic' }}>
                Danh sách phòng để thêm xe
              </Typography>
            </Box>
                ...
                rooms.map((room) => (
                  <Box
                    ...
                    sx={{
                      ...
                      mb: 2,
                      cursor: 'pointer',
                      bgcolor: selectedRoomId === room.roomId ? '#e3f2fd' : 'white',
                      transition: 'all 0.2s',
                      '&:hover': {

...
const InvoiceManager = ({ setIsAdmin, setIsNavAdmin, motels, setmotels }) => {
  ...
  const dynamicServiceColumns = useMemo(() => {
    const serviceColumnWidth = services.length === 2 ? 177 : services.length === 3 ? 118 : 100
  ...
  const StatusFormatter = (cell) => {
    ...
    if (status === 'Đã bị hủy') bgColor = '#B0B0B0'
    ...
    <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f7fa' }}>
      ...
      >
        ...
        <Box sx={{ px: '10px', pb: '16px', position: 'relative' }}>
          <InvoiceTable columns={columns} data={filteredData} options={options} />
